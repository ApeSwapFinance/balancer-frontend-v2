...
<template>
  <BalCard shadow="none" exposeOverflow noBorder>
    ...
    <div class="mb-6">
      <div class="text-gray">{{ $t('yourBalanceInPool') }}</div>
      <div class="font-bold text-lg">
        {{ hasBpt ? fiatTotalLabel : '-' }}
      </div>
    </div>
    <PoolInfoBreakdown :pool="fromPool" :poolTokenInfo="fromPoolTokenInfo" />
    <div class="block flex justify-center dark:text-primary-bright my-4">
      <ArrowDownIcon class="dark:text-gray-dark h-5 w-5" />
    </div>
